...
public class S3CanonicalArtifactStore implements CanonicalArtifactStore {
    ...
    public S3CanonicalArtifactStore(
            S3Client s3Client,
    ...
    public WriteResult put(
        ...
        if (sizeBytes < 1 || sizeBytes > maxBytes) {
            throw new ObjectStorageException(
                    ...
                    "Canonical object exceeds storage limit"
            );
        }
        ...
        try {
            ...
            if (measured.count() != sizeBytes) {
                throw unavailable("Canonical object size does not match", null);
            }
            return new WriteResult(measured.count(), measured.sha256());
        } catch (ObjectStorageException exception) {
            throw exception;
        } catch (S3Exception | SdkClientException exception) {
            throw unavailable("Canonical object could not be stored", exception);
        }
    }
    ...
    public InputStream open(String objectKey) {
        try {
            ...
            throw unavailable("Canonical object could not be read", exception);
        }
    }
    ...
    public boolean exists(String objectKey) {
        try {
            ...
            throw unavailable("Canonical object existence could not be checked", exception);
    ...
    public void delete(String objectKey) {
        try {
            ...
            throw unavailable("Canonical object could not be deleted", exception);
        }
    }
    ...
    /** 写入网络流时同步计数和摘要，防止声明大小绕过 canonical 上限。 */
    private static final class CountingDigestInputStream extends FilterInputStream {
        ...
        private void record(byte[] buffer, int offset, int length) throws IOException {
            count += length;
            if (count > limit) {
                throw new IOException("Canonical object exceeds configured limit");
            }
